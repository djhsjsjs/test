...
module.exports = async function handler(req, res) {
  ...
  try {
    // Vercel's Node runtime normally exposes the raw request body for this function.
    // If a custom parser is enabled, configure it to preserve the raw body.
  ...
  try {
    ...
    if (event.type === 'invoice.payment_failed' && obj.subscription) {
      ...
      await stripe.subscriptions.update(obj.subscription, {
        metadata: { ...sub.metadata, premium_granted: 'false' }
      });
    }
