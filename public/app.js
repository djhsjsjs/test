  ...
  $('freeResult').innerHTML = `
    <div class="report-shell">
      ...
      <div class="metric-grid">
        ...
        <div class="metric"><span>Estimated fair range</span><strong>${money(r.fair_price_low,p.currency)} — ${money(r.fair_price_high,p.currency)}</strong></div>
        <div class="metric"><span>Potential overpayment</span><strong>${r.overpayment_low == null ? 'Not enough evidence' : `${money(r.overpayment_low,p.currency)} — ${money(r.overpayment_high,p.currency)}`}</strong></div>
      </div>
      <div class="free-summary"><p>${esc(r.summary || 'The available evidence is not strong enough for a confident conclusion.')}</p></div>
      <div class="two-col">
        <div><h3>What looks good</h3><ul>${(r.positives||[]).slice(0,2).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>
        <div><h3>What to consider</h3><ul>${(r.concerns||[]).slice(0,2).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>
      </div>
      <div class="soft-paywall">
        ...
        <div>
          ...
          <p>Unlock detailed evidence, the best place to buy, better-value alternatives, Buy/Wait/Compare guidance, full score breakdown and saved price targets.</p>
        </div>
        <button class="button primary" id="unlockInline">Unlock for $2.99/month</button>
      </div>
    </div>`;
  ...
  $('fullReport').innerHTML = `
    <div class="report-shell premium-report">
      ...
      <div class="metric-grid five">
        <div class="metric"><span>Current price</span><strong>${money(p.current_price,p.currency)}</strong></div>
        <div class="metric"><span>Fair price</span><strong>${money(r.fair_price_low,p.currency)} — ${money(r.fair_price_high,p.currency)}</strong></div>
        <div class="metric"><span>Potential overpayment</span><strong>${r.overpayment_low == null ? '—' : `${money(r.overpayment_low,p.currency)} — ${money(r.overpayment_high,p.currency)}`}</strong></div>
...
function openPaywall() {
  ...
  if (!stripe) {
    toast('Stripe is not configured yet. Add the publishable key in Vercel.', 'error');
  }
}
$('closeModal').onclick = () => $('payModal').classList.add('hidden');
$('notNow').onclick = () => $('payModal').classList.add('hidden');

$('paymentForm').addEventListener('submit', async (e) => {
  e.preventDefault();
  const email = $('email').value.trim().toLowerCase();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    $('payError').textContent = 'Enter a valid email.';
    return;
  }
  ...
  try {
    ...
    if (!elements) {
      ...
      elements = stripe.elements({
        ...
        appearance: {
          theme:'stripe',
          variables:{ colorPrimary:'#171714', borderRadius:'14px', fontFamily:'Inter, system-ui, sans-serif' }
        }
      });
      const payment = elements.create('payment');
      payment.mount('#paymentElement');
      $('payBtn').textContent = 'Confirm $2.99/month →';
...
async function waitForPremium() {
  for (let i=0; i<25; i++) {
    const r = await fetch(`/api/subscription-status?subscriptionId=${encodeURIComponent(subscriptionId)}`, { cache:'no-store' });
    ...
    if (d.premium) {
      ...
      if (!state.user) {
        state.user = { email:$('email').value.trim().toLowerCase(), createdAt:new Date().toISOString() };
      }
      renderFull(report);
      updateHeader();
      return;
  ...
  throw new Error('Payment was received, but Stripe has not confirmed the subscription yet. Please refresh in a minute.');
}
...
function updateHeader() {
  ...
  $('accountButton').textContent = user ? user.email.split('@')[0] : 'Account';
