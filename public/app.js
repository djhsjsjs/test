...
async function initStripe() {
  try {
    ...
    if (typeof stripe.initEmbeddedCheckout !== 'function') {
      throw new Error('This Stripe.js build does not support Embedded Checkout. Please hard-refresh the page and redeploy the latest version.');
    }
    return stripe;
  } catch (err) {
...
async function mountCheckout() {
  if (checkout) return;
  const ready = await stripeReady;
  if (!ready || !stripe || typeof stripe.initEmbeddedCheckout !== 'function') {
    throw new Error('Stripe Embedded Checkout is not ready. Refresh the page and try again.');
  }
