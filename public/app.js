...
async function initStripe() {
  ...
  if (!window.Stripe) throw new Error('Stripe.js did not load. Please refresh the page and try again.');
  const r = await fetch('/api/config', { cache: 'no-store' });
  if (!r.ok) throw new Error('Could not load Stripe configuration.');
  const cfg = await r.json();
  if (!cfg.stripePublishableKey) {
    throw new Error('Stripe publishable key is missing in Vercel. Add DEALCHECK_STRIPE_PK to Environment Variables.');
  }
    ...
    <div class="report-shell">
      ...
      <div class="soft-paywall">
        ...
        <div>
          ...
          <div class="offer-inline"><b><s>$2.99</s> $0.99 today</b><span>3-day Premium starter</span><i>After the 3-day trial, the regular Premium plan renews at $35/month.</i></div>
        </div>
...
async function mountCheckout() {
  ...
  try {
    ...
    if (!stripe || typeof stripe.initEmbeddedCheckout !== 'function') {
      throw new Error('Stripe Embedded Checkout is unavailable. Check the Stripe publishable key and Stripe.js version.');
    }
