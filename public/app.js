  ...
  $('freeResult').innerHTML = `
    <div class="report-shell">
      ...
      <div class="metric-grid">
        ...
        <div class="metric"><span>Estimated fair range</span><strong>${fair ? `${money(r.fair_price_low,p.currency)} — ${money(r.fair_price_high,p.currency)}` : 'Not enough evidence'}</strong></div>
        <div class="metric"><span>Potential overpayment</span><strong>${over ? `${money(r.overpayment_low,p.currency)} — ${money(r.overpayment_high,p.currency)}` : 'Not enough evidence'}</strong></div>
      </div>
      ...
      <div class="soft-paywall">
        ...
        <div>
          ...
          <div class="offer-inline"><b>$0.99 today</b><span>3-day Premium starter</span><i>then $35/month</i></div>
        </div>
...
function openPaywall() {
  ...
  if (!stripe) {
    toast('Secure checkout is not configured yet. Add the Stripe publishable key in Vercel.', 'error');
    return;
  }
  mountCheckout();
}
...
async function mountCheckout() {
  ...
  try {
    ...
    localStorage.setItem('dealcheck_checkout_session_id', data.sessionId);
    localStorage.setItem('dealcheck_customer_id', customerId);
    checkout = await stripe.initEmbeddedCheckout({
      fetchClientSecret: async () => data.clientSecret,
      onComplete: async () => {
        await finishCheckout(data.sessionId, name, email);
      }
    });
    checkout.mount('#checkoutElement');
    $('payBtn').classList.add('hidden');
    $('notNow').classList.add('hidden');
  } catch (err) {
    $('payError').textContent = err.message || 'Secure checkout could not be loaded.';
    $('payBtn').disabled = false;
    $('payBtn').textContent = 'Start Premium for $0.99 →';
  }
}

async function finishCheckout(sessionId, name, email) {
  try {
    const r = await fetch(`/api/checkout-session-status?sessionId=${encodeURIComponent(sessionId)}`, {cache:'no-store'});
    const d = await r.json();
    if (!r.ok || !d.complete) throw new Error(d.error || 'Checkout is not complete yet.');
    if (!d.premium) throw new Error('Payment is processing. Please wait a moment and try again.');
    subscriptionId = d.subscriptionId;
    customerId = d.customerId;
    localStorage.setItem('dealcheck_subscription_id', subscriptionId || '');
    localStorage.setItem('dealcheck_customer_id', customerId || '');
    localStorage.setItem('dealcheck_premium', 'true');
    state.user = { name: d.name || name, email: d.email || email, createdAt: state.user?.createdAt || new Date().toISOString() };
    ...
    $('payError').textContent = err.message || 'Payment completed, but Premium is still being confirmed.';
  }
}

$('paymentForm').addEventListener('submit', async (e) => {
  e.preventDefault();
  // Checkout is embedded below. This button is only used to validate identity
  // and mount Checkout; the actual payment button is rendered by Stripe.
  const name = $('customerName').value.trim();
  const email = $('email').value.trim().toLowerCase();
  if (!name) return $('payError').textContent = 'Enter your full name.';
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return $('payError').textContent = 'Enter a valid email.';
  $('payError').textContent = '';
  mountCheckout();
});
...
async function resumeCheckoutFromUrl() {
  ...
  try {
    ...
    if (r.ok && d.premium) {
      localStorage.setItem('dealcheck_premium','true');
      localStorage.setItem('dealcheck_subscription_id', d.subscriptionId || '');
      localStorage.setItem('dealcheck_customer_id', d.customerId || '');
      state.user = { name:d.name || '', email:d.email || '', createdAt:state.user?.createdAt || new Date().toISOString() };
      if (report) renderFull(report);
      updateHeader();
      toast('Premium is active.', 'success');
    }
  } catch (_) {}
